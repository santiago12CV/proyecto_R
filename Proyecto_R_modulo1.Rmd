...
head(yi)
```
Ahora, ya teniendo todos los datos anteriores generaremos 1000 el mismo proceso calculando en cada uno de las veces cuatro diferentes estimadores para la pendiente:

Para hacer este proceso mil veces utilizaremos un FOR el cual cambie el número de semilla desde 1 hasta 100, calculando cada vez los cuatro estimadores. Necesitaremos que dichos resultados encontrados sean insertados en un vector diferente para cada estimador, por ello, procedemos a crear dichos vectores y una tabla/matriz donde agruparlos.
...
Una vez hecho esto, procederemos a crear el loop que llene dichos vectores con los resultados encontrados para cada diferente semilla, calculados para cada estimador y reagrupandolos en la tabla.
...
Para confirmar cual de los dos muestra las características de insesgamiento y eficiencia relativa buscaremos su varianza  y cual de ellos mediante el estimador de la media logra estimar de mejor manera y más acertada el parámetro poblacional, que en este caso es 5, cumpliendo con el insesgamiento.

```{r}
#Medias del estimador c) y d) respectivamente.
meanB_beta_u <-  mean(B_beta_u)
meanB_beta_2mco <- mean(B_beta_2mco)
MEDIAS <- cbind(meanB_beta_u,meanB_beta_2mco)
rownames(MEDIAS) <- c("media")
...
VARIANZAS <- cbind(var_betau,var_betamco2)
rownames(VARIANZAS) <- c("varianza")
MEDIAS
VARIANZAS
```
Teniendo lo siguiente podemos comprobar la hipotesis anteriormente planteada, ya que, el estimador d) es decir, el de MCO, es el estimador insesgado y de mínima varianza de los estimadores anteriormente planteados.
